...
input name: "minLevel", type: "number", title: "Nível mínimo útil (%) — 1% do slider mapeia para cá; 0% SEMPRE desliga", defaultValue: 0, range: "0..99"
}
...
def on() { allTo(100) }
def off() { allTo(0) }
private void allTo(int levelPct) {
...
Integer wire = parseFieldInt(f[2 + MAX_INPUTS + j])
if (wire == null) continue
def cd = getChildDevice(dimDni(j + 1))
if (!cd) continue
String sw = (wire > 0) ? "on" : "off"
if (wire > 0) anyOn = true
Integer lvl = wireToPct(wire)
List<Map> evts = [] 
if (cd.currentValue("switch") != sw) {
evts << [name: "switch", value: sw, descriptionText: "${cd.displayName} ${sw}"]
}
if (lvl > 0 && cd.currentValue("level") != lvl) {
...
void componentOff(cd) { sendLevel(cd, 0) }
...
private void sendLevel(cd, int levelPct) {
...
caSend("mdcmd_sendmd,${state.macDec},${ch - 1},${pctToWire(levelPct)}") 
}
